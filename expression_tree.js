const precedence_test = ( incoming , incoming_char , op_stack ) => {
	let current_rank = 0
	...
	if ( do_comparison ) {
		if ( incoming > current_rank ) {
			...
			postfix_notation.push( op_stack[ op_stack.length - 1 ] )
			op_stack.pop()
			op_stack.push( incoming_char )
		} else {
			...
			precedence_test( incoming , incoming_char , op_stack )
	...
	for (var i = 0 ; i <= equation.length; i++) {
		...
		if ( ( /[0-9A-Z\.]/g ).test( equation[i] ) ) {
			...
			if ( postfix_op_stack.length == 0 ) {
				...
				postfix_op_stack = precedence_test( incoming_rank , equation[i] , postfix_op_stack )
			}
		} else if ( expression != "" ) {
			postfix_notation.push( expression )
			expression = ""
...
const calculate_precentages = ( baseVal , precentages ) => {
	let base = baseVal
	let change = 0
